...
function loadAdminProducts() {
    ...
    products.forEach(prod => {
        ...
        row.innerHTML = `
            <td>${prod.id}</td>
            <td><img src="${prod.image}" alt="${prod.name}" width="50" style="object-fit: cover; border-radius: 4px;"></td>
            <td><strong>${prod.name}</strong></td>
            <td>${typeof getCategoryName === 'function' ? getCategoryName(prod.category) : prod.category}</td>
            <td>${typeof formatCurrency === 'function' ? formatCurrency(prod.price) : prod.price}</td>
            <td>${prod.stock}</td>
            <td>
                <button type="button" class="btn btn-sm btn-outline edit-product" data-id="${prod.id}">Sửa</button>
                <button type="button" class="btn btn-sm btn-danger delete-product" data-id="${prod.id}">Xóa</button>
            </td>
        `;
        tbody.appendChild(row);
    });

    // BẮT BUỘC: Phải gắn lại sự kiện cho các nút Sửa/Xóa NGAY SAU KHI in HTML ra
...
function editProduct(id) {
    ...
    window.scrollTo(0, 0); // Cuộn lên đầu trang để dễ sửa
}
...
function deleteProduct(id) {
    ...
    loadAdminProducts();
    if(typeof showToast === 'function') showToast('Đã xóa sản phẩm', 'success');
}

// KHỞI TẠO TẤT CẢ SỰ KIỆN KHI DOM ĐÃ SẴN SÀNG
document.addEventListener('DOMContentLoaded', () => {
    loadAdminProducts(); // Tự động load danh sách sản phẩm
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (id) {
                ...
                if (index >= 0) {
                    ...
                    if(typeof showToast === 'function') showToast('Cập nhật sản phẩm thành công', 'success');
                }
            } else {
                // Thêm
                const newId = products.length > 0 ? Math.max(...products.map(p => p.id)) + 1 : 1;
                products.push({ id: newId, ...productData });
                if(typeof showToast === 'function') showToast('Thêm sản phẩm thành công', 'success');
            }
